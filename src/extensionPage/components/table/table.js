...
function createData(name, dateUploaded) {
  return {
    name,
    dateUploaded,
  };
...
export default function EnhancedTable({
  ...
  const handleDeleteFromTable = (filesToDelete) => {
    handleDelete(filesToDelete);
    setSelected([]);
  };

  const handleExportFromTable = (filesToExport) => {
  ...
  const emptyRows = page > 0 ? Math.max(
    0, (1 + page) * rowsPerPage - rows.length,
  ) : 0;
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar
          ...
          handleDelete={() => handleDeleteFromTable(selected)}
          handleExport={() => handleExportFromTable(selected)}
        />
        <TableContainer>
          <Table
            sx={{ minWidth: 750 }}
